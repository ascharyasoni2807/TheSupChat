...
class StorageRepo {
  ...
  Future uploadChatPic(
      File file, otherUid, basenames, _imageUploadProvider) async {
    var user = await _auth.currentUser;
    var imageurl;
    ...
    firebase_storage.Reference reference1 = firebase_storage
        .FirebaseStorage.instance
        .ref()
        .child("chatImages/${otherUid}_${user.uid}");
    // firebase_storage.FirebaseStorage.instance.ref().child("chatImages/${user.uid}_${otherUid}/${user.uid}"+"_"+otherUid);
    firebase_storage.Reference reference2 = firebase_storage
        .FirebaseStorage.instance
        .ref()
        .child("chatImages/${user.uid}_${otherUid}/${user.uid}" +
            "_" +
            otherUid +
            random.toString() +
            "_" +
    ...
    if (reference1 == null && reference2 == null) {
      firebase_storage.Reference reference = firebase_storage
          .FirebaseStorage.instance
          .ref()
          .child("chatImages/${otherUid}_${user.uid}/${user.uid}" +
              "_" +
              otherUid +
              random.toString() +
              "_" +
  ...
  Future<List<String>> uploadFiles(
      List _images, otheruid, ImageUploadProvider _imageUploadProvider) async {
    print(_images);
    print(File(_images[0].path));
    _imageUploadProvider.setToLoading();
    var imageUrls = await Future.wait(
        _images.map((_image) => uploadFile(File(_image.path), otheruid)));
    print(imageUrls);
    _imageUploadProvider.setToIdle();
